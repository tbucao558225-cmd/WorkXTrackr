# models/attendance_data.py - COMPLETE FIXED VERSION
"""
Attendance Model
"""
...
class Attendance:
    ...
    def clock_in(self, user_id):
        """Save clock in to database - ALLOW MULTIPLE PER DAY"""
        try:
            ...
            print(f"⏰ CLOCK IN ATTEMPT:")
            print(f"   User ID: {user_id}")
            print(f"   Date: {today}")
            print(f"   Time: {clock_in_only}")
            print(f"   Status: {status}")

            # Check if we need to remove unique constraint first
            # First, check if there's a UNIQUE constraint on (user_id, date)
            try:
                # Try to insert new record (allow multiple per day)
                query = """
                ...
                if result:
                    print(f"✅ New clock in saved! Record ID: {result}")
                    return {"success": True, "record_id": result, "clock_in_time": current_time, "status": status}
                else:
                    print("❌ Failed to save clock in")
                    return {"success": False}

            except Exception as unique_error:
                if "Duplicate entry" in str(unique_error) and "unique_user_date" in str(unique_error):
                    print("⚠️  UNIQUE constraint blocking multiple entries per day")
                    print("   Removing the unique constraint...")

                    # Remove the unique constraint
                    self.db.execute_query("ALTER TABLE attendance DROP INDEX unique_user_date")
                    ...
                    if result:
                        print(f"✅ New clock in saved after removing constraint! Record ID: {result}")
                        return {"success": True, "record_id": result, "clock_in_time": current_time, "status": status}
                    else:
                        return {"success": False}
                else:
...
# models/attendance_data.py - ADD THIS HELPER METHOD
